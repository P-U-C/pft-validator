...
**7d window:** 2026-04-25T18:00:02Z to 2026-05-02T18:00:02Z  
**30d window:** 2026-04-02T18:00:02Z to 2026-05-02T18:00:02Z  
**Task ID:** 88bc90fe-a58b-4c31-b53c-104d66cfe3eb

### Reproducibility Note

This snapshot was computed from a local public-ledger index generated by BFS-crawling the PFT ledger from seed accounts. The database is not a privileged dashboard — it is a reproducible public-chain derivative. Any contributor can reproduce it by running the open-source indexer against the live PFT ledger. Future snapshots should either publish the indexer code publicly, publish an aggregate JSON export, or include the exact ledger range and crawl parameters used.
...
| Herald conversion rate | 4 of 36 delivered messages (11%). 58 total contacts but 22 failed delivery (no encryption key). | CRM calculation | Observed (B) |
...
|------|---------|---------|
| **A** | Directly computed from public chain data | memo_transactions_7d, total_accounts |
| **B** | Derived from chain data with labeling assumptions | active_contributors_7d (depends on bot labels), pft_distributed_7d (depends on infra wallet labels) |
| **C** | Manually observed from public surfaces | repo_commits, dashboard_live_status |
| **D** | Unavailable / proposed | validator_uptime, task_completion_rate |

### Network Health

| Metric | Value | Trend Signal |
|--------|-------|-------------|
| Sybil accounts flagged | 743 | 733 bot-labeled + 10 cluster-linked seed/intermediary accounts. Single cluster, traced to Feb 17, 2026 seeding event |
...
|-------------|--------------|----------|------------|----------------|
| Public awareness | Alpha artifacts + dashboards live | 40+ artifacts, Lens/Scanner/SUBS live | C | Distribution surface exists |
| Trial / first contact | New contributors (7d) | 8 | B | New users entering the network |
| Activation | Active contributors (7d) | 52 | B | 33% of humans active weekly |
| Work throughput | Memo transactions (7d) | 1,021 | A | Task/memo activity is live |
| Economic participation | Reward recipients (7d) | 53 | B | Rewards reaching broad set |
| Paid demand | Herald paying subscribers | 5 | A | Early but real demand-side signal |
| Retention | UNAVAILABLE | -- | D | Highest-priority missing metric |

This directly answers: **is activity compounding?** The funnel shows a working pipeline from awareness through economic participation. The critical gap is retention — we cannot yet measure whether contributors who start in week N are still active in week N+4.

---

## Metric Contamination Register

| Metric | Main Contamination Risk | Current Mitigation | Remaining Weakness |
|--------|------------------------|-------------------|-------------------|
| total_accounts | Bot seeding / sybil clusters | Report humans separately | Bot labeling can drift if classifier isn't re-run |
| total_transactions | Wash trading / infra churn | Prefer memo transactions | Memo spam still possible |
| active_7d | Wallets, not verified humans | Exclude bot + infra labels | Pseudonymous contributors may use multiple wallets |
| pft_distributed_7d | Airdrops mixed with task rewards | Infrastructure-wallet filter | Reward intent not fully typed (airdrop vs task) |
| Herald conversions | Tiny sample size (n=4) | On-chain payment verification | Denominator ambiguity (delivered vs contacted) |
| repo_commits | Manual count, not API-verified | Marked as confidence C | May drift between snapshots |
...
```json
{
  ...
  "windows": {
    "seven_day_start_utc": "2026-04-25T18:00:02Z",
    "thirty_day_start_utc": "2026-04-02T18:00:02Z"
  },
  "metrics": {
    "human_contributors": 156,
    "active_contributors_7d": 52,
    "new_contributors_7d": 8,
    "memo_transactions_7d": 1021,
    "daily_average_memos_7d": 145.9,
    "reward_recipients_7d": 53,
    "pft_distributed_7d": 3729827,
    "bot_accounts_flagged": 733,
    "sybil_cluster_accounts": 743,
    "herald_paying_subscribers": 5,
    "herald_conversions": 4
  },
  "unavailable_fields": [
    "validator_uptime",
    "unl_membership",
  ...
  "privacy_posture": "aggregate_only_no_individual_balances_no_geography_no_doxxing"
}
```

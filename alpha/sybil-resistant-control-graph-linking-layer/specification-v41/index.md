---
layout: default
title: "Task Authorization Gate Specification v4.1"
date: 2026-03-15
...
**Predecessor:** [Authorization Gate v4.0](../../network-task-authorization-gate-specification/specification-v4/)
...
The core architecture rests on five mechanisms:
...
5. Network coverage map with gap-filling incentives — the CGI rewards contributors (and inadvertently, sybil farms) for filling underserved domains. The optimal adversarial strategy becomes indistinguishable from the optimal cooperative strategy.
...
This should be stated clearly, because speaking Model B language while implementing Model A controls produces resentment. The network is honest about being curated. It invites contribution, but it does not promise that all contribution is equal.
...
The single most effective deterrence against early-state extraction is not detection — it is delayed liquidity.

A contributor who cannot receive liquid PFT until they have demonstrated alignment over time has limited incentive to optimize for burst extraction.
...
Sophisticated operators gain trust first — they invest the time to reach AUTHORIZED or even TRUSTED state, then exploit the higher-value surfaces those states unlock.

The most damaging extraction happens at high authorization levels, not low ones, because AUTHORIZED contributors have uncapped epoch weights and access to Tier 3 Alpha tasks.
...
Detection is useful, but it is secondary containment — not the core defense. The core defense is:
...
Signal accumulation requires enough data to distinguish sustained alignment from a lucky streak — one week is too noisy, one quarter is too slow to respond to drift.

Gaming resistance requires that burst activity in the final days of an epoch cannot meaningfully shift the epoch-long baseline.

Monthly cadence aligns with natural human and organizational reporting rhythms.

Inflation predictability: "this month's PFT supply" is a legible statement for contributors and external observers.
...
Starts at 1.0. Increases when a contributor's verified value weight in epoch N exceeds their trailing average. Decreases when output or alignment declines relative to own history. Decays 10% per epoch of inactivity after a 2-epoch grace period — prevents banking a high index during dormancy and returning at elevated weight.

#### 5.5.1 Network Coverage Map

The CGI calculation incorporates a real-time network coverage map — an aggregate view of signal density per domain, derived from the rewards ledger.

Each domain in the task taxonomy is assigned a coverage score:

Coverage score (domain D) = verified completions in D (trailing 2 epochs) / network target for D

A score below 0.6 designates the domain as a gap. A score above 1.4 designates it as saturated.

The coverage map is published at epoch open and does not change mid-epoch (prevents gaming through intra-epoch domain switching).

#### 5.5.2 Gap-Filling Multiplier

The CGI calculation applies a multiplier based on the contributor's domain distribution relative to the coverage map:

| Contribution profile | CGI adjustment |
|---------------------|----------------|
| ≥60% of completions in gap domains (coverage <0.6) | +0.3 CGI boost, applied to next epoch |
| Mix of gap and balanced domains | No adjustment |
| ≥60% of completions in saturated domains (coverage >1.4) | −0.1 CGI drag, applied to next epoch |

The boost and drag are bounded: CGI cannot exceed 3.0 or fall below 0.5 regardless of domain distribution.

A new contributor whose history is too sparse to classify (fewer than 5 completions) is treated as domain-neutral — no boost or drag.

Why this works against coordinated farms: A sybil operator running N wallets earns maximum CGI by distributing wallets across gap domains. This is exactly the behavior the network wants — gap coverage. The attack surface is neutralized because the optimal adversarial strategy produces genuine network value. Concentrating all wallets in one domain to maximize individual throughput earns the saturation drag, not the gap boost.

Why this is fair to legitimate contributors: The coverage map is public at epoch open. Legitimate contributors can see which domains are thin and choose to work there. The gap boost is an incentive, not a penalty for specialization — a contributor who has deep expertise in a saturated domain still earns at their standard CGI; they simply do not receive the additional boost. Steering toward gaps is the network's preference, not a requirement.

#### 5.5.3 Persona-Fit Routing

For contributors without sufficient history to demonstrate domain fit, the network offers gap-aware task routing: when requesting a new task, the contributor's task queue is weighted toward gap domains that overlap with their declared interests (Alignment Statement, Section 8.1) or their earliest task selections.

This is a soft nudge, not a hard constraint. The contributor can still request tasks outside the suggested domains. The routing simply surfaces gap-domain tasks first.

As history accumulates (10+ completions), the routing shifts from declared preference to demonstrated capability — the system infers fit from alignment scores and reviewer feedback on completed tasks, not from self-declaration alone.

This closes the gameable surface in early routing without creating a bootstrapping deadlock for genuinely new contributors.
...
[TRUST LEDGER UPDATE: slow-moving, path-dependent]
                              ↓
Epoch Close → [LINKING LAYER: composite score → provisional grouping → cap enforcement]
                              ↓
[CONCENTRATION CAP CHECK (per declared + provisional control graphs) → VESTING MECHANICS → SETTLEMENT]
```
...
### 12.1 Behavioral Fingerprinting Heuristics

The linking layer operates on observable behavior, not identity claims. It does not need to know *who* controls a wallet — it needs to detect whether two wallets behave as if controlled by the same entity.

Four primary heuristics are defined.

#### Heuristic 1: Task Submission Cadence Clustering

What it measures: Temporal correlation between submission timestamps across wallets. An operator controlling multiple wallets tends to submit tasks in bursts separated by idle periods — the operator's own activity rhythm. Independent contributors have uncorrelated rhythms.

Detection logic: For each pair of wallets (A, B), compute the cross-correlation of their hourly submission counts over a trailing 28-day window. A cross-correlation coefficient ≥ 0.72 at lag 0 (same-hour submissions) or lag ±1 (adjacent hours) is flagged as a cadence match.

Threshold: Single-pair flag if correlation ≥ 0.72. Control graph suspicion if ≥ 3 wallet pairs in a candidate graph all exceed 0.65.

False-positive mitigation: Cadence correlation is elevated during network-wide events (epoch opens, new task batch drops, protocol announcements). The baseline is dynamically adjusted by computing the 90th-percentile cross-correlation across all wallet pairs in the same epoch and subtracting it from individual pair scores. Correlation that merely reflects network-wide timing is not flagged.

#### Heuristic 2: Verification Artifact Similarity Scoring

What it measures: Structural and semantic similarity between task completion artifacts (written responses, code submissions, analysis documents) submitted by different wallets. A single operator tends to reuse phrasing, structural templates, and analytical frameworks across wallets even when varying surface content.

Detection logic: Completion artifacts are embedded into a 768-dimensional semantic vector space. Cosine similarity between artifact pairs is computed within each task type. Pairs with cosine similarity ≥ 0.81 on non-trivial tasks (Tier 2 and above, where artifacts are substantive) are flagged. For Tier 1 tasks, a higher threshold of 0.91 applies because short-form completions have inherently less discriminating surface.

Additionally, a shallow structural fingerprint is computed from document features (paragraph length distribution, heading depth, list frequency, punctuation density). Structural fingerprint similarity ≥ 0.78 is a secondary signal that can combine with lower semantic similarity to produce a flag.

Threshold: Two artifact pair flags from the same wallet pair within one epoch constitute a linking signal. One flag alone is logged but not actioned.

False-positive mitigation: Task prompts that are prescriptive (narrow format requirements) naturally produce similar artifacts across all contributors. Artifact similarity is normalized against the intra-epoch median similarity for the same task type. Only similarity that exceeds the normalized distribution is scored.

#### Heuristic 3: Reward Withdrawal Pattern Correlation

What it measures: Timing and destination correlation in on-chain reward withdrawals. Operators controlling multiple wallets often consolidate to a shared address, withdraw in the same transaction window, or use a common intermediary wallet pattern.

Detection logic: For each pair of wallets, the linking layer monitors:

- Destination overlap: If two wallets send funds to the same address (direct or within two hops), this is a strong linking signal regardless of timing.
- Withdrawal timing correlation: Pearson correlation of daily withdrawal amounts over trailing 56 days. Correlation ≥ 0.68 is flagged.
- Amount pattern similarity: Normalized withdrawal amount histograms compared via Jensen-Shannon divergence. Divergence < 0.15 (highly similar distributions) is a secondary flag.

Threshold: Destination overlap is a high-confidence flag that alone triggers a control graph audit. Timing correlation requires corroboration from at least one other heuristic.

False-positive mitigation: Epoch-end withdrawal clustering is a network-wide behavior — many contributors withdraw shortly after epoch settlement. Timing correlation is computed on a residual basis after removing the network-wide epoch settlement spike.

#### Heuristic 4: Memo Field N-gram Analysis

What it measures: Linguistic fingerprints in free-text fields (task memos, alignment statements, appeal notes, founder's pledge text). Individual writers have distinctive n-gram distributions that persist across pseudonyms.

Detection logic: Trigram and 4-gram frequency profiles are extracted from all text produced by each wallet (minimum 500 tokens required for reliability). Cosine similarity of n-gram frequency vectors is computed between wallet pairs. Similarity ≥ 0.74 across a corpus of ≥500 tokens per wallet is a linking signal.

Threshold: N-gram match flags are treated as medium-confidence — they do not trigger action alone but count toward the composite linking score.
...
Linking score = (1.5 × withdrawal_destination_overlap) + (1.2 × cadence_correlation_flag) + (1.0 × artifact_similarity_flag) + (0.8 × ngram_similarity_flag) + (0.5 × structural_fingerprint_flag)

Linking thresholds:

- Score ≥ 1.5: Soft flag — logged, monitored, no action
- Score ≥ 2.5: Suspected link — contributor prompted to amend control graph declaration (14-day window)
- Score ≥ 3.5: High-confidence link — wallets treated as same control graph for concentration cap purposes pending review; automated declaration amendment request with 7-day deadline
- Score ≥ 4.5 or withdrawal destination overlap present: Immediate provisional grouping; manual review triggered

---

### 12.2 Staking-Based Identity Binding

Behavioral detection is probabilistic. Staking creates an economic cost for running undeclared wallets even when detection fails.

#### 12.2.1 Registration Stake

Every wallet registering to participate (signing the Founder's Pledge) must post a registration stake in PFT or an accepted stable equivalent:

| Authorization state target | Minimum stake | Lockup period |
|---------------------------|--------------|---------------|
| UNKNOWN (entry) | 10 PFT equivalent | Until PROBATIONARY advancement or 60-day expiry |
| PROBATIONARY → AUTHORIZED path | 50 PFT equivalent | Until AUTHORIZED badge minted + 30 days |
| Automation stack registration | 200 PFT equivalent | Until deregistration |

Stakes are held in a protocol-controlled escrow, not by the core team. They are returned in full when the contributor exits the system in good standing.

The stake requirement is denominated in PFT equivalent rather than USD to create a supply-absorbing mechanism: as PFT price rises, the dollar cost of sybil registration rises proportionally without parameter changes.

#### 12.2.2 Slashing Conditions

Stakes are slashed (partially or fully forfeited to the protocol treasury) under the following conditions:

| Condition | Slash amount | Who authorizes |
|-----------|-------------|----------------|
| Wallet flagged as undeclared member of a known control graph, failed to amend after 14-day notice | 50% of stake | Automated |
| High-confidence linking score (≥4.5) confirmed by manual review as undeclared control | 100% of stake | Core team |
...
The economic model for an operator attempting to bypass the 15% cap via undeclared wallets:

- Each additional wallet requires a registration stake (minimum 10 PFT equivalent at entry)
- Each wallet must independently demonstrate sufficient history to reach PROBATIONARY (14 days minimum, 10 completions)
- PROBATIONARY wallets earn only 25% liquid — 75% vests and is forfeited if the wallet is suspended
- If detected (linking score ≥ 3.5), all wallets in the suspected graph are consolidated for cap purposes, negating the bypass, and the stake is at risk

For a 5-wallet sybil operation targeting a 15% cap bypass:

- Capital at risk: 5 × 50 PFT equivalent stake (PROBATIONARY path)
- Time cost: 14 days × 5 wallets of parallel operation
- Detection-adjusted expected value: (1 − P(detection)) × marginal reward gain − P(detection) × (stake loss + unvested credit forfeiture)

At current parameters, this expected value is negative unless P(detection) < ~0.15 and the marginal reward gain is substantial. The stake mechanism does not need to make sybil impossible — it needs to make it unprofitable in expectation, which it does.

---

### 12.3 Escalation Workflow

When the linking layer fires, the response must be calibrated to confidence level, extractor type, and authorization state of the suspected wallets.

#### 12.3.1 Automated Cooldown (Score 2.5–3.4)

At this confidence level, the system takes a conservative action that does not harm a legitimate multi-wallet operator:

1. Declaration prompt: Each wallet in the suspected pair/cluster receives a notification: *"Our monitoring has detected behavioral patterns suggesting these wallets may share a control graph. If you operate multiple wallets, please review and update your control graph declaration within 14 days. No action will be taken if the declaration is updated accurately."*
2. Provisional monitoring elevation: The wallets are added to an enhanced monitoring queue — more frequent heuristic re-scoring, lower flag thresholds for subsequent epochs.
3. No reward impact at this stage: The 15% cap is not yet applied across the suspected wallets unless the score escalates.

#### 12.3.2 Manual Review Trigger (Score 3.5–4.4)

1. Provisional cap grouping: The suspected wallets are treated as a single control graph for concentration cap purposes in the current epoch. Their combined weight is capped at 15%. This is applied immediately, not after review, to prevent extraction during the review window.
2. Core team review initiated: A review ticket is created with the composite linking score, per-heuristic breakdown, and the full behavioral evidence package.
3. Contributor notification: The contributor(s) are informed that their wallets have been provisionally grouped and review is underway. The 14-day declaration amendment window remains open.
4. Review SLA: 5 business days for core team determination.

#### 12.3.3 High-Confidence Escalation (Score ≥ 4.5 or destination overlap)

1. Immediate provisional grouping (same as 3.5+ but with higher urgency flag)
2. Stake at risk notification: Contributors are notified that slashing may apply
3. Expedited review: 2 business day SLA
4. If confirmed undeclared: Stakes slashed per Section 12.2.2, control graph declaration updated in the registry, epoch settlement recalculated with the unified cap applied retroactively within the epoch

#### 12.3.4 Appeal Process

Any contributor subject to provisional grouping or slashing may appeal within 30 days:

1. Submit appeal: Statement explaining why the flagged wallets are not under common control, plus supporting evidence (different team members' identity verification, distinct infrastructure documentation, etc.)
2. Review: Core team adjudication within 7 business days
3. Outcome: Full reversal (grouping removed, stake returned if slashed), partial reversal (grouping maintained but stake returned), or upheld
4. No re-appeal: A second appeal on the same determination requires new material evidence

Appeals are logged publicly (wallet addresses redacted) to build a precedent record that improves future calibration.

#### 12.3.5 Integration with Authorization State Machine

The linking layer feeds into the authorization state machine as follows:

- A provisional grouping flag appears on all affected wallets in the Authorization Registry (`linking_flag` field, added to schema in Section Appendix)
- If provisional grouping is confirmed, the control_graph field for all affected wallets is updated to reference the primary wallet of the group
- If a wallet in the group is AUTHORIZED or TRUSTED, a trust ledger deduction is applied (−0.15 trust score) for undisclosed common control — not for the control graph itself, but for the declaration violation
- TRUSTED wallets confirmed to have operated undeclared associated wallets are subject to demotion review, since TRUSTED advancement requires ZK identity verification and the undisclosed wallets imply that verification may have been circumvented

---

### 12.4 Adversarial Worked Examples

#### Example A: Sophisticated Extractor — 5 Undeclared Wallets

Setup: An operator registers 5 wallets (W1–W5) independently, each signing the Founder's Pledge as unaffiliated individuals. The goal is to capture 5 × 15% = 75% of the epoch budget while appearing as five independent contributors.

Epoch 1 behavior: All five wallets are UNKNOWN. They complete Tier 1 tasks at moderate pace — not fast enough to trigger velocity signatures individually. The operator is careful to vary submission timing across wallets by ±2 hours.

What the linking layer sees:

- Cadence correlation (H1): The ±2h variation is insufficient to avoid the 28-day cross-correlation window. W1–W5 all show elevated correlation (0.68–0.74). Score contribution: 1.2 per pair for the three pairs that cross 0.72.
- Artifact similarity (H2): Completion artifacts share structural fingerprints (paragraph length distribution, list frequency). Semantic similarity is moderate (0.75–0.82 on Tier 1 tasks, just below the 0.81 threshold). Structural fingerprint flag fires on 4 pairs. Score contribution: 0.5 per pair.
- Withdrawal patterns (H3): During UNKNOWN state, withdrawals are zero (0% liquid). No signal.
- N-gram analysis (H4): Memo fields and alignment statements share n-gram profiles (similarity 0.71). Just below the 0.74 threshold for standalone action.

Composite score after Epoch 1: Highest pair score = 1.2 (cadence) + 0.5 (structural) = 1.7. Soft flag. No action, but wallets enter enhanced monitoring.

Epoch 2 behavior: Wallets advance to PROBATIONARY. Now earning 25% liquid. Operator maintains timing variation but cannot avoid the cadence correlation over the extended window.

What the linking layer sees in Epoch 2:

- Cadence correlation: Extended window increases correlation. 4 of 10 pairs now exceed 0.72. Score: 1.2 per pair.
- Withdrawal timing (H3): First liquid withdrawals occur. Three wallets withdraw within the same 4-hour window to different addresses, but address graph analysis reveals that two destination addresses share a common second-hop address. Destination overlap flag fires for W1–W3. Score: +1.5.
- Artifact similarity (H2): Tier 2 tasks now available. Substantive artifacts show semantic similarity of 0.83–0.88 across W1–W5. Flag fires on 7 pairs.

Composite score after Epoch 2: W1–W3 pair score = 1.5 (destination overlap) + 1.2 (cadence) + 1.0 (artifact) = 3.7. Triggers Section 12.3.2: provisional cap grouping applied to W1–W3.

W4–W5 pair score = 1.2 + 1.0 = 2.2. Triggers Section 12.3.1: declaration prompt sent.

Outcome: The operator's W1–W3 cluster is capped at 15% combined in Epoch 2. W4–W5 remain uncapped temporarily but are under elevated monitoring. By Epoch 3, the extended behavioral window flags W4–W5 as well, and the full 5-wallet cluster is consolidated.

The operator captures at most 15% of the epoch budget — the same as a single declared control graph — despite five wallets.

The 25% liquid / 75% vesting split means the unvested credits from Epochs 1–2 are at risk when the slash is applied. The stake for each wallet (5 × 50 PFT equivalent) is slashed at 50% for the three wallets where linking was confirmed.

Total extraction: marginal, well below the target.

---

#### Example B: Legitimate Multi-Wallet Operator — Team of 4

Setup: A four-person research team (T1–T4) registers as a declared control graph. They disclose all four wallets in their Founder's Pledge, designate T1 as the primary wallet, and describe themselves as a team entity. Each team member contributes independently to different task domains.

Epoch behavior: T1 (quant analyst) submits Tier 2 analysis tasks. T2 (developer) submits Tier 2 technical tasks. T3 (writer) submits Tier 1 and Tier 2 documentation tasks. T4 (researcher) submits Tier 3 Alpha tasks under team authorization.

What the linking layer sees:

- Cadence correlation (H1): T1 and T2 work similar hours (same timezone) — cadence correlation of 0.69. Flagged for individual pair. However, T3 and T4 work different hours, reducing the cluster-level signal.
- Artifact similarity (H2): Task domains are different enough that artifact similarity is low (0.42–0.55). No flag.
- Withdrawal timing (H3): The team consolidates rewards weekly to a shared treasury wallet — destination overlap flag would fire.
- N-gram analysis (H4): Different authors, distinct n-gram profiles. No flag.

Composite score: T1–T2 pair = 1.2 (cadence). Destination overlap for the treasury consolidation = 1.5. T1–T2 composite = 2.7. This would normally trigger a declaration prompt.

But: The control graph declaration already exists. When the linking layer queries the registry, it finds that T1–T4 are declared as a single control graph entity. The destination overlap is expected — they declared a shared treasury. The cadence correlation between T1 and T2 is consistent with same-timezone teammates.

System response: The linking layer marks the correlation as explained by declaration and logs it as a false-positive candidate for calibration. No action taken.
...
### 12.5 Integration Points with Authorization Gate and Task Node Scoring Pipeline

The linking layer is not a standalone component — it feeds into and receives from the existing spec at five integration points:

1. **Authorization Registry (Section Appendix):** Two fields added — `linking_flag` (boolean, active provisional grouping), `suspected_control_graph_id` (string, reference to the provisional group if flagged). The registry is the source of truth for current grouping status.

2. **Eligibility Ledger (Section 2.1):** When a wallet's `linking_flag` is active, its eligibility score update is paused — the ledger does not advance the wallet's state while the review is pending. This prevents a flagged wallet from reaching AUTHORIZED while under investigation.

3. **Rewards Ledger (Section 2.2):** At epoch settlement, the rewards ledger applies the 15% cap at the control-graph level, not the wallet level. For flagged wallets, the provisional `suspected_control_graph_id` grouping is used. For declared wallets, the `control_graph` declaration is used.

4. **Concentration Cap Enforcement (Section 6.3):** The 15% cap mechanism now consumes the control graph groupings from both declared control graphs *and* provisional linkages from this section. A wallet with an active `linking_flag` is treated as part of its suspected group for cap purposes from the moment the flag is raised at score ≥ 3.5.

5. **Task Node scoring pipeline:** Task Nodes that evaluate submission quality receive the `linking_flag` status as part of contributor context. A flagged contributor's submissions are routed to an independent reviewer pool (no shared reviewers with the suspected group) to prevent mutual-boost rings from operating within a flagged cluster.
...
| linking_flag | bool | Active provisional control graph grouping from linking layer |
| suspected_control_graph_id | string | Provisional group ID if linking_flag is active |
| linking_score | float | Most recent composite linking score (highest pair score in graph) |
| linking_heuristics | string[] | Heuristics that fired in most recent linking evaluation |
| registration_stake | float | Current staked amount in PFT equivalent |
| stake_slashed_at | timestamp | If stake was slashed, when |
| stake_slash_reason | string | Reason for slash if applicable |
